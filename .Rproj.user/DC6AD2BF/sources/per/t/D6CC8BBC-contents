...
##---------------------------------------------------------
# Looking at the cumulative consumption of mussels by PO
...
total.cum.consumption <- master.number %>% 
  filter(trial.type == "Heat stress" | trial.type == "Recovery") %>% 
  group_by(water.treatment, air.treatment, trial.type, day.of.trial) %>% 
  summarise(total.cum.number.mussels = sum(cum.mussels),
            total.number.mussels = sum(mussels.consumed))

changepoint.master <- total.cum.consumption %>% 
  subset(select = c(air.treatment, water.treatment, day.of.trial, total.cum.number.mussels, total.number.mussels))

changepoint.master$day.of.trial <- as.numeric(changepoint.master$day.of.trial)
changepoint.master$total.cum.number.mussels <- as.numeric(changepoint.master$total.cum.number.mussels)
changepoint.master$total.cum.number.mussels <- as.numeric(changepoint.master$total.number.mussels)
...
###THE SAME CHANGEPOINT DAYS WERE DETECTED USING BOTH CUMULATIVE AND TOTAL MUSSEL CONSUMPTION
...
total.cum.consumption.PLOTTING <- total.cum.consumption %>%
...
total.cum.consumption.PLOTTING <- total.cum.consumption.PLOTTING %>%
...
total.cum.consumption.PLOTTING$changepoint <- factor(total.cum.consumption.PLOTTING$changepoint,
                                            ordered = TRUE,
                                            levels = c("Before", "After"))

##Add new rows to remove the gap in the data when plotting
##15 water and 20 air
total.cum.consumption.PLOTTING[nrow(total.cum.consumption.PLOTTING) + 1,] <- 
...
#################################################
# Cumulative consumption per treatment per day
#################################################

#Making the plot
cumconsumption_Plot_bytreatment <- total.cum.consumption.PLOTTING %>% 
  ggplot(aes(x = day.of.trial, y = total.cum.number.mussels, color = Treatment, linetype = changepoint)) +
  ...
  labs(x = "Day of experiment", y = "Cumulative number of mussels consumed per treatment") +
  theme(text = element_text(family = "A"),
        axis.title.x = element_text(vjust = -1, size = 12),
...
cumconsumption_Plot_bytreatment
...
#Making the plot
totalconsumption_Plot_bytreatment <- total.cum.consumption.PLOTTING %>% 
...
##########################################################################
# Difference in cumulative consumption between heat stress and recovery
...
MS_cumulativefeeding_exp1 <- cumconsumption_Plot_bytreatment + changepoint.plot +
